...
export default function EditRolePage({ params }) {
  ...
  const fetchRole = async (token, roleId) => {
    setLoading(true);
    setError("");
    
    try {
      const response = await fetch(`http://localhost:3232/role/${roleId}`, {
      ...
      if (data.status && data.data) {
        ...
        setPermissions({
          can_add_company: role.can_add_company || false,
          can_add_user: role.can_add_user || false,
          can_assign_device: role.can_assign_device || false,
          can_view_data: role.can_view_data || false,
          can_view_log: role.can_view_log || false,
          can_manage_iot: role.can_manage_iot || false
        });
      } else {
        setError(data.message || "Rol bulunamadı");
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.status) {
        ...
        setError(data.message || "Rol güncellenirken bir hata oluştu");
      }
    } catch (err) {
      setError("Sunucu hatası");
    } finally {
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <h2 className="text-2xl font-bold text-text-primary">Rol Düzenle</h2>
      ...
      <div className="bg-dark-100 border border-border-primary rounded-2xl shadow-md p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <button type="submit" disabled={submitting} className="px-4 py-2 rounded-lg border border-accent-primary bg-accent-primary text-white hover:bg-accent-primary/90 transition">
